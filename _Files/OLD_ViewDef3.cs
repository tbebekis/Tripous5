...
namespace Tripous.Data
{
    ...
    public class ViewDef3 
    {
        ...
        public ViewDef(SqlBrokerDef Broker, UiSplit Split = null)
        {
            ...
            // PageList pages
            ViewTabDef FilterTab = this.AddTab("Filters", "Filters");       // filters (search) tab
            ViewTabDef ListTab = this.AddTab("List", "List");               // List (browse) tab
            ViewTabDef EditTab = this.AddTab("Edit", "Edit");               // Edit tab (contains a tab pager, i.e. its tabs is not empty)

            // Edit TabControl pages
            ViewTabDef DataTab = EditTab.AddTab("Data", "Data");       // the single tab page of the Edit pager
            DataTab.TableName = Broker.MainTableName;

            // the single row of the Data tab-page
            ViewRowDef Row = DataTab.AddRow(DataTab.TableName); 
